/**
 * POST /api/bookings/[id]/generate-payment-link
 * Génère un lien de paiement et l'envoie par email au client
...
const log = childLogger({ route: 'api/bookings/[id]/generate-payment-link' });
...
export async function POST(
  ...
  return handleApiRoute(request, async () => {
    ...
    if (!isAdmin) {
      ...
      return { error: 'Accès refusé. Seuls les administrateurs peuvent générer un lien de paiement.' };
    }
    ...
    log.info({ bookingId, userId: session.user.id }, 'Generating payment link');
    ...
    try {
      ...
      const updatedMetadata = {
        ...
        paymentLinkGeneratedBy: session.user.id,
      };
      ...
      return {
        success: true,
        message: 'Lien de paiement généré et envoyé avec succès',
        paymentUrl: checkoutResult.url,
        emailSent,
      };
    } catch (error) {
      log.error({ error, bookingId }, 'Error generating payment link');
      throw error;
    }
  }, 'api/bookings/[id]/generate-payment-link');
}
