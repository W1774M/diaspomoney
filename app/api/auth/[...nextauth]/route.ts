  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      ...
      if (debug) {
        ...
        console.log("[AUTH][signIn] providerAccountId=", account?.providerAccountId);
      ...
      try {
        ...
        if (!existing) {
          const baseUrl = (process.env["NEXTAUTH_URL"] as string) || "http://localhost:3000";
          ...
          if (debug) console.log("[AUTH][signIn] Email inconnu → redirection vers:", redirectUrl);
          return redirectUrl;
        }
        ...
        if (provider === "google") {
          ...
          update.$set["oauth.google.providerAccountId"] = account?.providerAccountId;
        ...
        if (provider === "facebook") {
          ...
          update.$set["oauth.facebook.providerAccountId"] = account?.providerAccountId;
        }
        ...
        console.error("[AUTH][signIn] erreur lors de la vérification/liaison:", e);
        return false;
      }
    },
    async jwt({ token, user, account, profile }) {
      ...
      if (user) {
        token["userId"] = (user as any).id ?? (user as any)._id ?? token["userId"];
      }
      if (profile) {
        const p: any = profile;
    ...
    async session({ session, token }) {
      ...
      if (session.user) {
        ...
        (session.user as any).id = (token as any).userId ?? (session.user as any).id;
        session.user.image = (token as any).picture ?? session.user.image;
        // Indiquer les liaisons éventuelles (utile pour les settings UI)
        (session as any).linkedProviders = (session as any).linkedProviders || {};
      }
      const debug = (process.env["AUTH_DEBUG"] ?? "").toLowerCase() === "true";
      if (debug) {
