...
export const SECURITY_CONFIG = {
  ...
  SECURITY_HEADERS: {
    ...
    'Content-Security-Policy': "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https://api.diaspomoney.fr;"
  }
};
...
export class SecurityManager {
  ...
  validatePassword(password: string): { isValid: boolean; errors: string[] } {
    ...
    if (password.length > config.maxLength) {
      errors.push(`Le mot de passe ne peut pas dépasser ${config.maxLength} caractères`);
    }
  ...
  generateTokens(user: { id: string; email: string; role: string }) {
    const accessToken = jwt.sign(
      ...
      { expiresIn: SECURITY_CONFIG.JWT_EXPIRES_IN, algorithm: 'HS256' }
    );
    ...
    const refreshToken = jwt.sign(
      ...
      { expiresIn: SECURITY_CONFIG.JWT_REFRESH_EXPIRES_IN, algorithm: 'HS256' }
    );
  ...
  async verifyToken(token: string, type: 'access' | 'refresh' = 'access'): Promise<any> {
    try {
      const secret = type === 'access' ? SECURITY_CONFIG.JWT_SECRET : SECURITY_CONFIG.JWT_REFRESH_SECRET;
  ...
  authenticate = async (request: NextRequest): Promise<{ user: any; error?: string }> => {
    try {
